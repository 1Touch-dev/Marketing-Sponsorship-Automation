...
- [ ] **Remaining route retrofit — real, substantial work, not started beyond the 2 proven entities.** ~148 of the 150 total mutating routes still query without tenant scoping — same `.eq("tenant_id", auth.user.tenant_id)` pattern now proven correct on companies/proposals, needs mechanically applying to the rest (campaigns, emails, contracts, inventory, templates, etc.). This is the largest remaining piece of Phase 4 by volume.
- [x] **Strip hardcoded Coritiba-specific content out of AI prompts — started, 1 of ~15 done.** `CORITIBA_CONTEXT` (the block injected into every text-generation prompt) converted from a static string into `buildClubContext(tenant)`, verified byte-for-byte equivalent output for Coritiba before/after. ~14 more prompt functions in `lib/bedrock/prompts.ts` (proposal generation, campaign ideas, all outreach emails, pricing tiers, image prompts, fit analysis — ~113 more scattered "Coritiba FC"/"Couto Pereira" mentions) still hardcode the club by name in their own system-prompt lines; not yet touched. The deck/presentation template hasn't been touched either.
...
- [ ] White-label theming per tenant — **not started.** `tenants.branding` (JSONB: colors, logo, crest) exists in the schema from migration 0047, but no UI surface reads or applies it yet — the whole app UI is still hardcoded Coritiba green/white regardless of which tenant is logged in.
