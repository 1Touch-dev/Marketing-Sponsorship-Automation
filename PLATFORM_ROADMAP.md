...
- [~] **Follow-up from the RBAC audit above** — in progress, 2026-09-09. Two sub-tracks:
  ...
  - [~] **`requirePermission()` rollout to session-protected-but-role-ungated routes** — batch 1/N done (24 routes): outreach agent approval gates, email send/generate/status, Gmail sync/disconnect, media/AI image generation, proposal/campaign generation. ~75 lower-priority routes remain (settings, templates, contacts, inventory, matches, intelligence enrichment, misc proposal sub-routes) — continuing in priority order (blast radius first, read-mostly config last).
