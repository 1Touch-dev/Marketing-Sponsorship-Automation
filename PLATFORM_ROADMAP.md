...
### Phase 1.5 — Gaps surfaced by a full historical-log audit — found 2026-09-18
A full pass through every dated log/WhatsApp-style status doc and planning file in the repo (not just `master_report.md`) turned up 5 real, still-open items that James asked for or a tester reported, which had never been folded into this roadmap. Recorded here so they don't fall through the cracks a second time.
- [ ] **Bug A — user gets logged out when pressing a button.** James relayed this from a team member on 17 July 2026 ("other team member tested and whenever he pressed a button he got logged out"). Likely cause noted at the time: Supabase auth session/middleware returning a hard 401→redirect on an expired/invalid cookie instead of refreshing the token. **Never reproduced — genuinely needs from James**: which page/button, and whether it happens every time or intermittently. Source: `17th_July.md` item A, `README.md` Known Pending Items #1.
- [ ] **Pipeline board has no drag-and-drop.** Moving a lead between pipeline stages requires opening the company and changing the stage dropdown by hand; dragging a card between Kanban columns was asked for repeatedly (first flagged `8th_June_E2E_test.md`, repeated `9th_june.md`, `26th_june.md`) and is still unbuilt. Source: `README.md` Known Pending Items #9.
- [ ] **Presentation templates, phase 2 — PPT/Google Slides upload + PDF/public-share-link export.** The HTML-template half of this ask shipped (16–17 July: industry-tagged templates, placeholder parsing, bulk auto-customize per company); James's original ask also included uploading a PowerPoint/Slides file directly, and exporting a rendered presentation to a PDF or a shareable public link (today it only produces a stored HTML file + URL). Explicitly scoped as "phase 2, not today" and never revisited. **Needs from James**: a real sample HTML template file to validate placeholder-naming conventions against actual usage, per his own follow-up ask. Source: `16th_July.md`, `17th_July.md` item C.
- [x] **AI-generated campaign images can now be assigned to a specific Marketing Campaign — done 2026-09-18.** James asked for this explicitly on 9 June ("Add select or choose images. Assign to each Marketing Campaign"). Migration 0051 added `image_generation_jobs.campaign_id`; assignment dropdown + filter on `/assets`, shown back on the campaign detail page. While wiring this up, found and fixed two serious pre-existing bugs (same class as the Pipeline page's `estimated_value` issue): the Asset Library page had been selecting a nonexistent `image_url` column (real fields are `output_urls[]`/`selected_url`) and a nonexistent `metadata` column, silently showing "0 Total Assets" against 214 real rows for an unknown period — no user had ever seen a real image in the Asset Library before this fix. Live-verified end to end: 200 real assets now render, a real assignment persisted and appeared correctly on both sides, unassign confirmed working.
- [ ] **`LOGO_DEV_TOKEN` has expired** (401 on every call) — the resolver already fails over to Apollo + Google favicon, so this isn't breaking anything today, but it means the platform is running on the lower-resolution logo tier. **Needs from James**: a free key from https://logo.dev/signup. Source: `README.md` Known Pending Items #3.
...
### Phase 8 — Agentic team expansion — **done 2026-09-18**, `master_report.md` Section 7
Explicit instruction from James: do not start this until hardening is done and validated — Phases 1–3 closed before this began.
- [x] **Team 1 — Sponsorship Outreach — done.** Discovery/Enrichment/Proposal already existed (`lib/agents/orchestrator.ts`, hardened in Phase 1) and was deliberately left untouched rather than migrated to LangGraph, per the framework decision below (production-critical, no durable cross-request checkpointer exists yet for a real `interrupt()` migration). **Negotiation Agent — done 2026-09-17**, the one role from the original 5-agent team never built (`lib/agents/langgraph/negotiation-agent.ts`): drafts a grounded counter-response when an inbound email is classified as an objection/needs-info, output is a normal draft email through the existing approval gate. Live-verified against a real email/proposal thread.
- [x] **Team 2 — CRM Outreach — done.** Pipeline Hygiene Agent (2026-09-17, `lib/agents/langgraph/pipeline-hygiene-agent.ts`) — found and fixed a severe pre-existing bug along the way: the whole Pipeline board was silently showing 0 of 540 real companies due to a query selecting a nonexistent column. Renewal Agent (2026-09-18, `lib/agents/langgraph/renewal-agent.ts`) — completes the "Renovar" flow the report references (`contracts.renewed_from_contract_id` existed in the schema since early on but was never referenced in application code); drafts a renewal proposal grounded in the real contract being renewed, live-verified against a disposable near-expiry test contract, idempotent per contract. Reporting Agent (2026-09-18, `lib/agents/langgraph/reporting-agent.ts`) — drafts monthly ROI report emails grounded in the real per-match reach data Phase 5's ROI dashboard already computes; correctly refuses to draft anything when no real reach data exists (true for both real contracts in the DB today), live-verified with disposable match/reach data. Multi-Channel Sequencer — the email channel already existed and works (`app/api/email-sequences/advance`, `/settings/email-flows`, confirmed still live 2026-09-18); WhatsApp/LinkedIn channels are correctly NOT claimed as built — no API credentials provisioned for either, same class of external blocker as Documenso/Stripe. All four roles surfaced on `/agents`.
- [x] **Unified post-setup orchestrator — done 2026-09-18**, `/agents` dashboard. Did not build a second approval queue — every agent above already writes `proposals.status="under_review"` / `emails.status="pending_approval"`, the exact statuses `/approvals` already queries tenant-wide, so Team 2's output was flowing into the shared queue by construction. The one new piece: `/api/agents/team2/run-all` combines Hygiene + Renewal + Reporting + due email-sequence steps behind one button (Team 1's agents already have natural per-item triggers on the company/email pages, so weren't included). Live-verified: real counts (86 proposals/20 emails/169 campaigns pending), Run All returned internally consistent results matching real DB state.
...
- [x] **Public read-only MCP server — v1 done 2026-09-18.** The report frames this as a 2-3 quarter-horizon item; built a genuinely working v1 rather than waiting, scoped honestly short of the full vision. `POST /api/mcp/public` (Streamable HTTP transport, stateless), 5 read-only tools (`list_companies`, `get_company`, `list_proposals`, `list_campaigns`, `list_inventory` — `lib/mcp/public-server.ts`), auth via a single bearer token (`MCP_PUBLIC_API_KEY`) rather than per-partner issued/revocable keys, since there's exactly one real external-consumer class today (n8n/agency workflows) and no real partner yet needing individual key management — that's the genuine remaining "2-3 quarter" gap, not the protocol/tooling itself. No PII (contact email/phone), no internal cost fields (`inventory_items.production_cost`), no raw proposal content exposed — metadata only. Live-verified with a real MCP SDK client end to end: `initialize` → `tools/list` → `tools/call` against all 5 tools, real data returned correctly scoped; auth correctly rejects missing/wrong bearer tokens (401).
...
- [x] Public MCP server — v1 done 2026-09-18 (see Phase 11)
