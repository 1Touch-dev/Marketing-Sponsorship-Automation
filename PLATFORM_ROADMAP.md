...
- [ ] **AI-generated campaign images can't be assigned to a specific Marketing Campaign.** James asked for this explicitly on 9 June ("Add select or choose images. Assign to each Marketing Campaign"). Checked the current codebase: image-generation jobs link to proposals/companies, not to the standalone Campaigns entity — no `campaign_id` linkage exists anywhere in the media-generation code or schema. Lower confidence than the other items here — it's possible this was intentionally superseded by proposal-level graphics rather than left open — but it was never explicitly closed out in any later log or in this roadmap, so flagging rather than assuming. Source: `9th_june.md` "Not Yet Implemented" table.
