...
- [ ] Supabase/Postgres (read-only, AI-assisted schema/reporting) — **investigated 2026-09-15, genuinely blocked, not attempted further.** The official Supabase MCP server authenticates with a Supabase **Personal Access Token** (account-level, generated in Supabase account settings) — a different credential from the project-level `SUPABASE_SERVICE_ROLE_KEY` already in `.env.local`. No PAT exists yet; needs James to generate one from the Supabase dashboard before this can be wired.
