...
- [ ] White-label theming per tenant (full UI restyle — every component's colors/logo/nav labels) — **not started, substantial standalone UI project.** `tenants.branding` (JSONB: colors, logo, crest) exists in the schema from migration 0047 and the proposal-facing surfaces above now read it, but the rest of the app UI (sidebar nav, dashboard chrome, every page's green/white styling) is still hardcoded Coritiba branding — distinct from the AI-content/proposal-template work above, which is done.
