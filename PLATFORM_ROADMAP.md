...
- [~] **Follow-up from the RBAC audit above** — in progress, 2026-09-09. Two sub-tracks:
  - [x] **`/api/internal/*` and `/api/system/*` unauthenticated-bypass fix — done.** These bypass the session middleware entirely by design (meant to be secured per-route via `requireInternalAuth()`), but 8 of 13 routes never called it — reachable with no login and no secret at all from the public internet. Two were actively dangerous (`internal/cleanup`'s `deduplicate_companies` deleted any company sharing a name with another, real data, unauthenticated DELETE; `system/maintenance` bulk-mutated real proposals/workflow_events). All 8 fixed and live-verified.
  - [~] **`requirePermission()` rollout to session-protected-but-role-ungated routes** — batches 1-2/N done (43 routes): outreach agent approval gates, email send/generate/status, Gmail sync/disconnect, media/AI image generation, proposal/campaign generation, campaign per-item actions (including `preapprove`, a real safety-gate bypass — flagging a campaign for the batch agent to skip human approval entirely, previously open to any logged-in user), company/intelligence enrichment (Hunter/Apollo/Apify/OpenAI cost). ~55 lower-priority routes remain (contacts, inventory, matches, contracts, newsletter, social-projects, barter, sender-profiles, team-members, warmup-*, email-sequences/templates, proposal-templates, misc proposal sub-routes) — continuing in priority order.
