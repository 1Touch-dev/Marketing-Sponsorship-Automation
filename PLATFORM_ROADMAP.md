...
- [ ] **Follow-up from the RBAC audit above**: extend `requirePermission()` (`lib/auth/server-permission.ts`) to the ~110 remaining mutating routes without a server-side check — settings writes (email flows, templates, sender profiles), campaigns, media generation, contracts, newsletter. Prioritize by blast radius: anything that sends real communications or spends real money next, read-mostly config last.
